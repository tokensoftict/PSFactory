...
class InvoicePolicy
{
    ...
    {
        if(!userCanView('invoiceandsales.view')) return false;
    ...
    {
        if(!userCanView("invoiceandsales.edit")) return false;
    ...
    {
        if(!userCanView("invoiceandsales.pos_print")) return false;
    ...
    public function showPayment(User $user, Invoice $invoice)
    {
        ...
        if(!(in_array($invoice->status_id, [status("Paid"),status("Dispatched")])))  return false;
