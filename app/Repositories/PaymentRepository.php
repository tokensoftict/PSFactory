...
class PaymentRepository
{
    ...
    */
    ...
    public function savePayment(&$obj) : void
    {
        ...
        $invoiceValidation = $this->invoiceRepository->validateInvoiceForPayment($obj->invoice);

        if($invoiceValidation['status'] === false)
        {
            $obj->emit('InvoiceError',['errors' => $invoiceValidation['errors']]);
            $obj->alert(
                "error",
                "Payment Error",
                [
                    ...
                    'timer' => 2500,
                    'toast' => false,
                    'text' =>  "One or more item in the invoice does not have enough quantity, please check and try again",
                ]
            );
            return ;
        }
        ...
        $obj->invoice->wipePayment();

        $payment = $this->addPayment($payment_data);

        $this->invoiceRepository->removeStockInInvoice($invoiceValidation['batches'], $invoiceValidation['items']); // remove stock after payment
