...
@section('pageHeaderAction')
    @if(app(\App\Classes\Settings::class)->get('m_run_nears') !== 'running' && app(\App\Classes\Settings::class)->get('m_run_nears') !== 'run')
        <div class="row">
            <div class="col-12">
                <a href="{{ route('run_nearos') }}"  class="btn btn-primary float-end">Run Material Near Os</a>
                <br/> <br/>  <br/>
            </div>
        </div>
    @else
...
@section('content')
    ...
    <livewire:raw-material.datatable.material-nearoutotstock :filters="$filters['filters']"/>
    </div>
